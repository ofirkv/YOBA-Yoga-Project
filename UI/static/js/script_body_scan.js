...
function capturePhotoAndCheck() {
  ...
  .then(data => {
    if (data.status === "ok") {
      ...
      // Continue to training page. Use route used by your Flask app.
      // If your route is '/train' use that, otherwise 'train_page.html' or '/ready' accordingly.
      window.location.href = '/train'; // change if needed
    } else {
      stopListening();
      detectedText.className = "pink";
      if (counter === 1)
